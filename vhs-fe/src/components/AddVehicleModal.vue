    ...
    <div class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="relative bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div class="p-6 sm:p-8 max-h-[calc(90vh-180px)] overflow-y-auto">
                ...
                <div v-show="step === 2" class="space-y-4">
                    <div class="bg-black/5 p-4 rounded-2xl border border-gray-200">
                        <label class="text-xs font-black uppercase text-gray-500 block mb-3">Vehicle Type Template</label>
                        <div class="flex flex-wrap gap-2">
                            <label v-for="t in ['Truck', 'Car', 'Bus']" :key="t" class="flex items-center gap-2 cursor-pointer px-3 py-2 rounded-lg transition" :class="presetType === t ? 'bg-black text-white' : 'bg-white hover:bg-gray-100'">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between items-center">
                            ...
                            <button type="button" @click="addDocumentRow" class="text-xs font-black text-blue-600 hover:underline">
                                + Add Custom
                            </button>
                        </div>
                        <div class="space-y-2 max-h-80 overflow-y-auto">
                            <div v-for="(doc, index) in form.documents" :key="index" class="flex gap-2 items-center bg-gray-50 p-3 rounded-xl border border-gray-200 hover:border-gray-300 transition">
                                ...
                                <button type="button" @click="removeDocumentRow(index)" class="text-gray-300 hover:text-red-500 transition flex-shrink-0">
            ...
            <div class="bg-gray-50 px-6 sm:px-8 py-4 border-t border-gray-200 flex gap-3 justify-end">
                <button @click="emit('close')" class="px-4 py-2 text-sm font-bold text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 transition">
                    Cancel
                </button>
                <button v-if="step === 2" @click="step = 1" class="px-4 py-2 text-sm font-bold text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 transition">
                    Back
                </button>
                <button v-if="step === 1" @click="step = 2" :disabled="!canProceed()" class="px-4 py-2 text-sm font-bold text-white bg-black rounded-lg hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition">
                    Next
                </button>
                <button v-if="step === 2" @click="handleSubmit" :disabled="loading || !canSubmit()" class="px-4 py-2 text-sm font-bold text-white bg-black rounded-lg hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition">
